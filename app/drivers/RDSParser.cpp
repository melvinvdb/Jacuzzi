...
RDSParser::RDSParser() {
	rdsGroupType = rdsTP = rdsPTY = _textAB = _last_textAB = 0;
	/*_sendServiceName = 0;
	_sendTime = 0;
	_sendText = 0;*/
...
void RDSParser::registerForCallback(RDSParserInterrupt& classCallBack)
{
	callBackServiceName = RdsReceiveServiceNameCallback(&classCallBack, &RDSParserInterrupt::RdsReceiveServiceName);
	callBackText = RdsReceiveTextCallback(&classCallBack, &RDSParserInterrupt::RdsReceiveText);
	callBackTime = RdsReceiveTimeCallback(&classCallBack, &RDSParserInterrupt::RdsReceiveTime);
...
void RDSParser::processData(uint16_t block1, uint16_t block2, uint16_t block3, uint16_t block4)
{
  ...
  if (block1 == 0) {
    ...
    if (callBackRegistered) callBackServiceName.execute(programServiceName);
    if (callBackRegistered) callBackText.execute("");
    return;
  } // if
      ...
      if ((idx == 6) && strcmp(_PSName1, _PSName2) == 0) {
        if (strcmp(_PSName2, programServiceName) != 0) {
          ...
          if (callBackRegistered)
        	  callBackServiceName.execute(programServiceName);
        } // if
      } // if
    } // if
  ...
  case 0x4A:
    ...
    if ((callBackRegistered) && (mins != _lastRDSMinutes)) {
      _lastRDSMinutes = mins;
      callBackTime.execute(mins / 60, mins % 60);
    } // if
    break;
